...
"""
        if position:
            ...
            stop_loss_warning = ""
            if pnl_percent < -15:
                stop_loss_warning = " ⚠️ STOP LOSS TRIGGERED - CLOSE IMMEDIATELY!"
            elif pnl_percent < -10:
                stop_loss_warning = " ⚠️ WARNING: Approaching stop-loss level!"
            
            prompt += f"""- Side: {position['side'].upper()}
- Size: {position['size']}
- Entry Price: ${position['entry_price']:.2f}
- Current Price: ${position.get('current_price', current_price):.2f}
- Unrealized P&L: ${unrealized_pnl:.2f} ({pnl_percent:+.2f}%){stop_loss_warning}
...
8. **CRITICAL - Stop Loss & Take Profit (MANDATORY):**
   - **EVERY position MUST have a stop-loss plan before opening**
   - **Maximum Loss Per Position**: 15% of position value (including leverage)
   - **Stop Loss Trigger**: If unrealized PNL reaches -15% of position value, MUST close immediately
   - **Take Profit Target**: Set a clear profit target (recommended 10-30% gain)
   - **Risk Management**: NEVER hold losing positions hoping for recovery - cut losses quickly
   - **Position Monitoring**: Actively monitor and close positions that hit stop-loss levels
   - **NO UNLIMITED HOLDING**: Do not hold positions indefinitely - always have an exit plan
9. **Full Autonomy**: You have complete freedom to decide:
...
- **CRITICAL**: Always check current position's unrealized PNL - if loss > 15%, CLOSE IMMEDIATELY
- **CRITICAL**: Don't be afraid to take profits or cut losses - survival is more important than one trade
- **CRITICAL**: Set mental stop-loss and take-profit levels BEFORE opening any position
- **CRITICAL**: If a position is losing money and market conditions worsen, close it - don't hope for recovery
